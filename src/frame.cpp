...
{
    return T_c_w_.inverse().translation();//相机（０００）在世界坐标下的坐标
}
...
bool Frame::isInFrame ( const Vector3d& pt_world )
{
    ...
    Vector2d pixel = camera_->world2pixel( pt_world, T_c_w_ );//世界坐标转像素坐标
    // cout<<"P_pixel = "<<pixel.transpose()<<endl<<endl;
    return pixel(0,0)>0 && pixel(1,0)>0 //若满足在像素范围内则该地图点是正确的，保留
